...
def qbin_pilatus(img_exp, Tthdet, Rdet, Gamdet, Xdet, Ydet, Samth=0.0, Samchi=0.0, Samphi=0.0, plotflag=True):
    """
    ...
    Detcen = np.array([
        Rdet * np.sin(tthd),
        Rdet * np.sin(gamd) * np.cos(tthd),
        Rdet * np.cos(gamd) * np.cos(tthd)
    ])
    ...
    jvec = np.array([np.cos(tthd), 0, -np.sin(tthd)])  # Cross product to get horizontal vector
    ...
    sqz = sROT @ np.array([0, 0, 1])  # 00L rod direction
    ...
    if Powdflag:
        ...
        for i, (y, x) in enumerate(zip(iref, jref)):
            if y >= 0:
                plt.text(x, y+100, f"Si[{refl[i, 0]}{refl[i, 1]}{refl[i, 2]}]", fontsize=10, color='white')
        for i, (y, x) in enumerate(zip(iref2, jref2)):
            if y >= 0:
                plt.text(x, y+100, f"Au[{refl2[i, 0]}{refl2[i, 1]}{refl2[i, 2]}]", fontsize=10, color='white')
        plt.show(block=False)
    ...
    np.savetxt("temp/tempm1_python.csv", tempm1, delimiter=",")
